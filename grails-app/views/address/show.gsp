	...
	<body>
		...
		<div id="show-address" class="content scaffold-show" role="main">
			...
			<ol class="property-list address">
				...
				<g:if test="${addressInstance?.group}">
				<li class="fieldcontain">
					<span id="group-label" class="property-label"><g:message code="address.group.label" default="Group" /></span>
					
						<span class="property-value" aria-labelledby="group-label"><g:link controller="group" action="show" id="${addressInstance?.group?.id}">${addressInstance?.group?.encodeAsHTML()}</g:link></span>
